...
const reduceFn =
	...
	<E, Args extends ArgsType<F>>(acc: Kind2<F, E, VOK<F, Args>>, curr: Kind2<F, E, unknown>) => {
		const merge =
			<A>(result: A) =>
...
export const concurrency2 =
	<F extends URIS2>(mo: Instance<F>) =>
	(config?: Config) =>
	<Args extends ArgsType<F>>(args: Args): Kind2<F, UnionE<F, Args>, VOK<F, Args>> => {
		...
		while (remaining.length > 0) {
			...
			acc = pipe(
				...
				mo.chain((values) =>
					pipe(portion as Kind2<F, E, unknown>[], L.reduce(mo.of(values), reduceFn(mo))),
				),
			);
		}
