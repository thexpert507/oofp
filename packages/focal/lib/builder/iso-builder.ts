...
import type { NullablePaths, OptionalKeys, PathValue, SafePaths } from "../path-types";
import { PendingUpdate } from "./pending-update";
import { Focal, Lens, Prism, Traversal, URIS } from "../index";
...
export class IsoBuilder<S, A> {
	constructor(public readonly focal: IFocal<Iso.URI, S, A>) {}
	...
	elements(this: IsoBuilder<S, A[]>): TraversalBuilder<S, A> {
		return new TraversalBuilder(pipe(this.focal, Focal.elements()));
	}

	prop<Key extends SafePaths<A>>(key: Key): LensBuilder<S, PathValue<A, Key>> {
	...
	optionalProp<K extends OptionalKeys<A> & string>(key: K): LensBuilder<S, A[K]> {
	...
	first<Elem>(this: IsoBuilder<S, Elem[]>, pred: Fn<Elem, boolean>) {
	...
	index<Elem>(this: IsoBuilder<S, Elem[]>, i: number) {
		return new PrismBuilder(pipe(this.focal, Focal.index(i)));
	}

	indexRecord<Elem>(this: IsoBuilder<S, Record<string, Elem>>, key: string) {
