...
export const createSuccess = (v: number) => E.right<string, number>(v);
export const createFailure = (e: string) => E.left<string, number>(e);

// ── Pipeline (5-step) ───────────────────────────────────────────
// parse → validate range → double → validate even → format
